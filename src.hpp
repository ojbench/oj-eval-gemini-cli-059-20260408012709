...
protected:
    bool CheckAnswer(const char *output) const override {
        ...
        while (*p1 || *p2) {
            ...
            const char* te1 = e1;
            while (te1 > p1 && (*(te1 - 1) == ' ' || *(te1 - 1) == '\r')) te1--;
            const char* te2 = e2;
            while (te2 > p2 && (*(te2 - 1) == ' ' || *(te2 - 1) == '\r')) te2--;

            if (te1 - p1 != te2 - p2) return false;
            for (int i = 0; i < te1 - p1; ++i) {
...
public:
    ...
    void Submit(size_t time, size_t memory, const char *output) override {
        ...
        size_t current_score = std::floor(time_score * memory_score / 100.0);
